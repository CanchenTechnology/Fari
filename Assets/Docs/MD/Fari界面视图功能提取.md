...
- 当前实现：
  ...
  - 会展示今日每日牌额度、今日对话额度，并根据后端会员状态刷新 Free/Pro 额度文案。
...
- 当前实现：
  ...
  - `NotionUI` 支持每日神谕、对话回复、占卜回访、好友互动、活动/系统通知开关，以及每日提醒时间切换。
...
- 当前实现：
  ...
  - Firebase Hosting 已配置 `web-admin`，可用 `MOONLY_DEPLOY_HOSTING=1 ./scripts/deploy-firebase.sh` 随后台一起部署。
...
- 根目录已新增 `scripts/finish-release.sh`，作为一键最终续跑入口。默认读取 `scripts/release.env`，也支持 `--no-env-file` 直接使用当前 shell 环境变量；它会强制执行 IAP secret 写入、Functions secret bindings 部署、iOS 导出、Android APK 构建和真实 sandbox receipt release gate；可用 `--dry-run` 预演。
...
- 根目录已新增 `scripts/setup-firebase-secrets.sh`，从环境变量读取 DeepSeek、TTS、Apple / Google IAP 和可选 webhook secret，并通过 Firebase CLI 写入 Functions Secrets，不把密钥写进仓库；Google 服务账号支持 `GOOGLE_SERVICE_ACCOUNT_JSON_FILE` 文件输入，并会校验 `type == service_account`、`client_email` 和 `private_key`。脚本也支持 `MOONLY_SECRET_NAMES` / `SET_SECRET_NAMES` 部分写入，比如只补当前缺失的 `APPLE_SHARED_SECRET,GOOGLE_SERVICE_ACCOUNT_JSON`。
...
- `functions/scripts/set-public-config.js` 已支持 `--dry-run` 写入前验证；会检查社媒链接、IAP 商品 ID、订阅类型、商品 ID 唯一性，完整 readiness 会自动执行 dry-run。
