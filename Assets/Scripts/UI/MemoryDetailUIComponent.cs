...
public class MemoryDetailUIComponent:MonoBehaviour
{
	...
	public Switch labelSwitchSwitch;
	public void InitComponent(WindowBase target)
	{
	    //组件事件绑定
	    target.Canvas.sortingOrder = (int)windowLayer;
	    target.Layer = windowLayer;
	    MemoryDetailUI mWindow=(MemoryDetailUI)target;
	    target.AddButtonClickListener(BackButton,mWindow.OnBackButtonClick);
	    target.AddButtonClickListener(EditMemoryButton,mWindow.OnEditMemoryButtonClick);
	    target.AddButtonClickListener(DeleteMemoryButton,mWindow.OnDeleteMemoryButtonClick);
	}
}
