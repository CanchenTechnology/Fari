...
public class DialogUI : WindowBase
{
	...
	{
		if (dialogSystem == null) return null;
		...
		if (msgData.messageType == DiaMessageType.User)
		{
			// 用户消息
			item = listView.NewListViewItem(userItemPrefabName);
			if (item == null)
			{
				Debug.LogError("Failed to create user item. Please check prefab name: " + userItemPrefabName);
				return null;
			}
			...
			if (chatItem == null)
			{
				chatItem = item.gameObject.AddComponent<ChatItem>();
			}
			chatItem.SetData(msgData);
		}
		else
		{
			// AI回复
			item = listView.NewListViewItem(aiItemPrefabName);
			if (item == null)
			{
				Debug.LogError("Failed to create AI item. Please check prefab name: " + aiItemPrefabName);
				return null;
			}
			...
			if (messageItem == null)
			{
				messageItem = item.gameObject.AddComponent<MessageItem>();
			}
			messageItem.SetData(msgData, OnAIOptionClick, OnRegenerateVoiceClick);
		}
