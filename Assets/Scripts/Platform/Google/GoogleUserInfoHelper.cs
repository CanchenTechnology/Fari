...
public class GoogleUserInfo
{
    ...
    public string photoUrl;         // 头像 URL
    public string providerId;       // 登录提供方，如 "google.com"
    ...
    [Header("账号状态")]
    public bool isEmailVerified;    // 邮箱是否已验证（Google 账号登录后通常为 true）
    ...
    {
        return $"[GoogleUserInfo] UID={firebaseUid}, Email={email}, Name={displayName}, " +
...
/// <summary>
/// 从 FirebaseUser 提取 Google 账号信息，并写入 UserDataManager 的工具类
/// 登录成功后直接调用，无需修改任何原生代码
/// </summary>
public static class GoogleUserInfoHelper
{
    #region 提取信息
    ...
    /// </summary>
    public static GoogleUserInfo GetCurrentUser()
    {
        var user = FirebaseAuth.DefaultInstance.CurrentUser;
        if (user == null)
        {
            Debug.LogWarning("[GoogleUserInfoHelper] 当前无 Firebase 登录用户");
            return null;
        }
    ...
    /// </summary>
    public static GoogleUserInfo ExtractFromFirebaseUser(FirebaseUser user)
    {
    ...
    /// <summary>
    /// 从 Google 提供方 ProviderData 中提取信息（更精准）
    /// 如果用户同时绑定了多种登录方式，专门取 google.com 那条记录
    /// 注意：GivenName / FamilyName 需通过原生 Google Sign-In 回调获取，这里取不到
    /// </summary>
    public static GoogleUserInfo ExtractFromGoogleProvider(FirebaseUser user)
    {
        if (user == null) return null;
        ...
        // 没找到 google.com 提供方，降级为主用户信息
        Debug.LogWarning("[GoogleUserInfoHelper] 未找到 Google 提供方数据，降级返回主用户信息");
        return ExtractFromFirebaseUser(user);
    }
    ...
    /// <summary>
    /// 提取 Google 用户信息并同步到 UserDataManager（一步到位）
    /// Google 登录成功后在 FirebaseAuthManager.OnLoginSuccess 回调里调用此方法即可
    /// 优先从 google.com ProviderData 取信息（更精准），失败则降级到主用户
    /// </summary>
    ...
    public static void SyncToUserDataManager(FirebaseUser user)
    {
        if (user == null)
        {
            Debug.LogWarning("[GoogleUserInfoHelper] SyncToUserDataManager: FirebaseUser 为 null，跳过");
            return;
        }

        GoogleUserInfo info = ExtractFromGoogleProvider(user);
        if (info == null)
        {
            Debug.LogWarning("[GoogleUserInfoHelper] SyncToUserDataManager: 无法提取用户信息");
            return;
        }

        Debug.Log($"[GoogleUserInfoHelper] 开始同步 Google 用户数据: {info}");

        UserDataManager.Instance.SyncFromGoogleUserInfo(info);
    }

    /// <summary>
    /// 直接将已提取的 GoogleUserInfo 同步到 UserDataManager
    /// </summary>
    public static void SyncToUserDataManager(GoogleUserInfo info)
    {
        if (info == null)
        {
            Debug.LogWarning("[GoogleUserInfoHelper] SyncToUserDataManager: GoogleUserInfo 为 null，跳过");
            return;
        }

        Debug.Log($"[GoogleUserInfoHelper] 同步 Google 用户数据: {info}");
        UserDataManager.Instance.SyncFromGoogleUserInfo(info);
    }
    ...
    /// <summary>
    /// 下载头像并缓存到本地（persistentDataPath/user_avatar.png）
    /// 优先读本地缓存，无缓存才走网络下载
    /// 用法：StartCoroutine(GoogleUserInfoHelper.LoadAndCacheAvatarCoroutine(url, sprite => avatarImage.sprite = sprite));
    /// </summary>
        ...
        bool forceRefresh = false)
    {
        ...
        using (var req = UnityWebRequestTexture.GetTexture(normalizedUrl))
        {
            ...
            {
                Debug.LogWarning($"[GoogleUserInfoHelper] 头像下载失败: {req.error}");
                onComplete?.Invoke(null);
            }
        }
    }
