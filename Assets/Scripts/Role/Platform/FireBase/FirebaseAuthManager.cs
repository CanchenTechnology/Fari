...
public class FirebaseAuthManager : MonoSingleton<FirebaseAuthManager>
{
    ...
    private void SignInAnonymouslyWithFirebase()
    {
        ...
        _auth.SignInAnonymouslyAsync().ContinueWithOnMainThread(task =>
        {
            ...
            StartLoginPreload(AuthProvider.Anonymous);
            FriendOnlinePresenceManager.Instance.StartPresence();
    ...
    private void CompleteEmailAuth(FirebaseUser newUser, string actionName)
    {
        ...
        StartLoginPreload(AuthProvider.Email);
        FriendOnlinePresenceManager.Instance.StartPresence();
    ...
    private void StartLoginPreload(AuthProvider provider)
    {
        try
        {
            LoginPreloadService.Instance.BeginAfterLogin(provider);
        }
        catch (Exception e)
        {
            Debug.LogWarning("[FirebaseAuthManager] 启动登录预热失败: " + e.Message);
        }
    }
