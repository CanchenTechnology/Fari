...
//
// C# callback contract (same as Android MoonlySignInActivity):
//   SUCCESS:<idToken>
...
static NSString* MoonlyGetIOSClientID(void) {
...
static void MoonlySendToUnity(NSString* go, NSString* method, NSString* result) {
...
extern "C" void _moonlyRestorePreviousSignIn(const char* gameObjectName, const char* callbackMethod) {
    ...
    [GIDSignIn.sharedInstance restorePreviousSignInWithCallback:^(GIDGoogleUser *user, NSError *error) {
        ...
        MoonlySendToUnity(go, method, result);
    }];
}

#pragma mark - Interactive Sign-In

extern "C" void _moonlyStartGoogleSignIn(const char* webClientId, const char* gameObjectName, const char* callbackMethod) {
...
extern "C" void _moonlyGoogleSignOut(void) {
