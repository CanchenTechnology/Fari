...
| 可获取 | `displayName`、`email`、`photoUrl`、`birthday`、`city`、`timezone`、`membershipStatus`、`proExpiresAt` |
| 可修改 | 昵称、头像、时区、会员字段、`isAdmin`、`role`、资料更新时间 |
| 后台操作 | 选择 `users`，填写文档 ID 精确查询 UID，编辑 JSON 后保存 |
...
| 场景 | 建议 |
| --- | --- |
| 授权管理员 | 只添加 `isAdmin=true` 或 `role="admin"` |
| 会员申诉 | 先核对 `iap_receipts`，再谨慎修正会员字段 |
...
支付相关数据通常包括 `iap_receipts`、`payment_events` 和 `users/{uid}` 里的会员字段。它们共同支持会员状态判断和客服排查。
...
```bash
firebase deploy --only functions:adminDataCollections,functions:adminDataList,functions:adminDataUpsert,functions:adminDataDelete,hosting --project fari-app-b2fd2
```
